...
* 2026-07-12 - FEATURE: Dashboard Analytics Restructuring — Repurposed `SummaryChart.tsx` pie chart to visually tally dynamic restoration types parsed from submitted `toothConfigs` instead of case statuses, equipped with a configurable time-filter dropdown. Refactored Active Cases / Active Production cards in both `DentistDashboard` and `LabDashboard` to adopt a compact tabular status list (Pending, Active, Completed) replacing the large singular count.
* 2026-07-12 - FEATURE: Contiguous Teeth Selection Limit Refinement — Updated teeth selection checks to restrict choices based on a maximum contiguous group size of 6 in the same jaw, rather than absolute per-jaw limits. Added step-transition block checks, treatment-type shift warning popups, final submit validations, and sanitization of 'none' states in serialized design parameters.
* 2026-07-10 - FEATURE: Teeth Charting SVG Update — Refactored Dentist Dashboard teeth charting SVG to adopt the upper/lower vertical ellipse jaw layout from demo.html. Added conditional rendering of inward-facing implant screws and pink buccal veneer arcs. Repositioned bridge buttons outside the arches using ellipse math, removed unconnected dashed lines, and unified the restoration indications list globally. Removed the absolute-positioned Indications overlay legend.
